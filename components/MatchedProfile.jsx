...
const MatchedProfile = ({profile, removeFromMatchedUsers}) => {
    ...
    const handleThumbsDown = async () => {
        try{
            ...
            if(response.status === 200){
                removeFromMatchedUsers(profile.id);
    ...
    <div className='bg-white border border-light_gray p-5 shadow-md w-full rounded-lg md:w-1/2 lg:w-1/3' >
        <div className='flex items-center justify-start gap-5' >
            ...
            <div className='w-2/3' >
                ...
                <div className='flex items-center justify-center gap-2 mt-2' >
                    <button className='bg-us_blue flex items-center justify-between gap-2 text-white px-2 py-1 text-sm rounded-lg cursor-pointer' ><ChatIcon /></button>
                    ...
                    <button className='bg-us_blue px-2 py-1 rounded-lg flex items-center cursor-pointer justify-center text-white' onClick={toggleThumbsDownOpen} >
                        <CancelIcon />
                    </button>
                    <button className='bg-us_blue flex items-center justify-between gap-2 text-white px-2 py-1 text-sm rounded-lg cursor-pointer' ><StarOutlinedIcon className='text-yellow-400' /></button>
                </div>
            </div>
        </div>
        ...
        {thumbsDownOpen && (
                ...
                <div className='bg-white px-6 py-12 rounded-2xl shadow-md' >
                    <p className='text-center text-dark_text text-md' >Are you sure you’re not interested in this profile? This profile will be removed from your matches and vice versa.</p>
